...
import { buildForecastSeries } from "./utils/forecastUtils";
import "./App.css";
...
function App() {
  ...
  const [dateRange, setDateRange] = useState(
    () => localStorage.getItem("gs_dateRange") || "30 Days"
  );

  const [regionFilter, setRegionFilter] = useState(
    () => localStorage.getItem("gs_region") || "All Regions"
  );

  const [categoryFilter, setCategoryFilter] = useState(
    () => localStorage.getItem("gs_category") || "All Categories"
  );
  ...
  useEffect(() => {
    localStorage.setItem("gs_dateRange", dateRange);
    localStorage.setItem("gs_region", regionFilter);
    localStorage.setItem("gs_category", categoryFilter);
  }, [dateRange, regionFilter, categoryFilter]);
...
function DashboardPage({
  ...
  const { series: forecastSeries, rSquared } = useMemo(
    () => buildForecastSeries(dateChartData, metricKey, 7),
    [dateChartData, metricKey]
  );
          ...
          <div className="chart-toolbar">
            ...
            <label className="forecast-toggle">
              <input
                ...
                onChange={(event) =>
                  setShowForecast(event.target.checked)
                }
              />
              Show 7-day forecast{" "}
              {showForecast && `(R² ${rSquared})`}
            </label>
